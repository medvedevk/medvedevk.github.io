...
function numberFixed(number, fixed) {
	...
	this.addToCart = function(curObj, id, params)
		...
		var id_ = ( $.isEmptyObject(params.subid) ) ? id : id + "_" + params.subid;
            ...
            if (params.discount && params.discount > 0) {
                if ('F' === params.typediscount )  {

                    discount = params.price - (numberFixed(params.price, 2) - params.discount);
                    priceDiscount = params.price - discount;
                    totalDiscount += params.discount;



                } else if ('P'=== params.typediscount) {

                	priceDiscount = params.price - ((params.price)/100 * params.discount);
                	discount = (params.price)/100 * params.discount;
                    totalDiscount += discount;

                }
            } else {
            	priceDiscount = params.price;
		...
		var goodieLine = {"id" : id_, "name" : params.name, "price": priceDiscount, "num" : kol, "url" : document.location.href, "photo" : "", "discount" : totalDiscount};
	...
	this.reCalc = function()
		...
		// *** //		 		
		this.widjetObj.html(local.goods + " " + num + " " + local.amount + " " + numberFixed(sum, 2) + " Руб.");
	...
	this.renderBasketTable = function()
		...
		if ($('#bcontainer').length == 0)
			...
			$("body").append(" \
				...
				<div id='bfooter'> <button class='bbutton' onclick=\"cart.showWinow('order', 1)\">" + local.order + "</button><span id='bsum'>...</span> <span id='bsun'> ...</span> <span id='bsune'> ...</span> </div> \
				</div> \
			");
		...
		for(var idkey in this.DATA) 
			...
			with (this.DATA[idkey])
				...
				var productLine = '<tr class="bitem" id="wigoodline-' + id + '"> \
													...
													<td id="lineprice_' + id + '"class="wigoodprice">' + numberFixed(price, 2) + ' руб.</td> \
													<td> \
													...
													<td id="linesum_' + id + '">'+ numberFixed(top, 1) + ' руб.</td> \
		...
		for(var ids in this.IDS)
			...
			$("#linesum_" + cartItemID).html( numberFixed((price * cartNum), 2) + ' руб.' );
	...
	this.sumAll = function()
		...
		for(var idkey in this.DATA) { sum += parseFloat(this.DATA[idkey].price * this.DATA[idkey].num);
			disc += parseFloat(this.DATA[idkey].discount);
}

		$("#bsum").html( "Итого " + numberFixed(sum, 2) + " руб.");
            $("#bsun").html("Скидка " + numberFixed(totalDiscount, 2) + " руб.");
            $("#bsune").html("Сумма без скидки " + numberFixed(sum + totalDiscount, 2) + " руб.");
		};
